...
tramite diversi ottimizzatori come Nevergrad e Nomad. Però può essere facilmente esteso per includere diversi
algoritmi.

In questa relazione di tirocinio si propone l'implementazione di un algoritmo genetico di OpenAI per risolvere problemi
blackbox.

Un algoritmo genetico è una procedure di ricerca euristica ispirate all'evoluzione naturale osservabile in natura.
...
Possiamo ottimizzare i parametri $\theta$ usando un gradient descend stocastico, vedere la figura \ref{gradient_descend} per una visualizzazione grafica del funzionamento.
...
Poiche il gradiente può essere molto lungo esso va scalato per un iper-parametro dell'algoritmo $\alpha > 0$ chiamato \textbf{learning rate}.
...
\centering
    ...
    \end{axis}
    \end{tikzpicture}
\caption{Visualizzazione del gradient descend}
...
\For{$t = 0, 1, 2, \dots$}
    ...
    \State $\theta_{t+1} \gets \theta_t - \alpha \frac{1}{\sigma n} \sum_{i=1}^n \mathcal{L}(\theta_t + \sigma \varepsilon_i) \varepsilon_i$
\EndFor
\end{algorithmic}
\end{algorithm}
