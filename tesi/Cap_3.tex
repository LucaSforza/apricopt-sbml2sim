...
Per vincolare definitivamente la stabilità bisogna eliminare il transitorio. Il transitorio è la parte
...
il valore delle concentrazioni tra 0 e 1, poiché per ipotesi le concentrazioni delle specie sono tutte normalizzate.
...
Per vincolare le specie tra 0 e 1 dobbiamo minimizzare il valore di queste variabili.

\begin{equation}
    \mathcal{L}_2(\theta) = \sum_{i=1}^n z_i(T, \theta)
\end{equation}

I parametri delle constanti cinetiche devono garantire non solo la stabilità, ma devono anche
alcune regole biologiche. Per esempio l'acqua è il solvente della vita, tutto ciò che
...
Un modo per vincolare i parametri a questi dati è massimizzando la \textbf{likelihood} \cite{Simpson2025} dei parametri.

La likelihood è la probabilità condizionata di osservare dal modello
...
I dati osservati $\mathcal{D}$ per ipotesi devono provenire da repliche sperimentali indipendenti ottenuti nelle stesse condizioni sperimentali del modello e affette da rumore osservazionale.
Si assume un errore additivo gaussiano sulle medie osservate:
\[
y_i = \tilde{x}_i(T,\theta) + \varepsilon_i,\qquad \varepsilon_i \sim \mathcal{N}(0,\sigma_i^2),
\]
dove \(\sigma_i^2\) è la varianza dell'errore per la specie \(i\). Sotto questa ipotesi la log-likelihood del dataset è
\[
...
\begin{equation}
    \mathcal{L}_4(\theta) = \sum_{(i,y) \in \mathcal{D}} (y_i - \tilde{x}_i(T,\theta))^2
\end{equation}
...
Dove $\alpha, \beta , \gamma, \zeta \in \mathbb{R}$ sono degli iper-parametri dell'ottimizzatore per scalare
...
\section{Ordinamenti plausibili delle specie}

Dato che stiamo lavorando in un ambiente normalizzato allora possiamo
cercare di capire se un certo ordinamento delle specie è plausibile o meno.

Un ordinamento lo possiamo vedere come una permutazione sulle specie $\pi: [1,n] \rightarrow [1,n]$

\begin{equation}
    \mathcal{D} = \left\{ \left( i,\frac{\pi(i)}{n+1} \right) \mid i \in [1,n]\right\},
\end{equation}

Vincoliamo le concentrazioni delle specie con valori normalizzati tra 0 e 1.

Per capire quale ordinemto sia più plausibile di un altro possiamo confrontare l'ottimizzazione della funzione rispetto a due permutazioni diverse.

\pagebreak

\section{Formulazione del problema di ottimizzazione vincolata}

Il problema di ricerca dei parametri che rendono un modello biologico plausibile può essere espresso formalmente come un problema di ottimizzazione vincolata.
L'obiettivo è trovare il vettore di parametri $\theta = (\theta_1, \dots, \theta_p)$ che minimizza la funzione di costo $\mathcal{L}$, la quale combina
i vari vincoli biologici, numerici e di stabilità del sistema.

\begin{equation}
    \hat{\theta} = \argmin_{\theta} \mathcal{L}(\theta)
\end{equation}

Dove $\mathcal{L}(\theta) = \alpha \cdot \mathcal{L}_1(\theta) + \beta \cdot \mathcal{L}_2(\theta) + \gamma \cdot \mathcal{L}_3(\theta) + \zeta \cdot \mathcal{L}_4(\theta) + \mathcal{L}_5(\theta)$.

Riassumendo i significati dei vari termini:

\begin{itemize}
    \item $\mathcal{L}_1$ penalizza la presenza di transitori, garantendo la stabilità del sistema.
    \item $\mathcal{L}_2$ vincola le concentrazioni normalizzate nell'intervallo $[0, 1]$.
    \item $\mathcal{L}_3$ impone relazioni di ordinamento biologicamente plausibili tra le specie.
    \item $\mathcal{L}_4$ misura l'aderenza ai dati sperimentali.
    \item $\mathcal{L}_5$ penalizza errori numerici di simulazione.
\end{itemize}

In più i parametri devono soddisfare l'ordinamento della relazione $R$ \eqref{eq:relazione}.

Il problema risultante è un'ottimizzazione black-box non lineare e non derivabile, risolta tramite metodi evolutivi o numerici (Nevergrad, Nomad, OpenAI Evolution Strategy).
La soluzione ottima \(\hat{\theta}\) rappresenta l'insieme di parametri che definiscono un virtual patient coerente con il modello biologico e i vincoli imposti.
