    ...
    double error(std::expected<SimulationResult,double> sim_result) const override {
        ...
        if(sim_result) {
            for (const auto& [handler, scale] : this->handlers) {
                double err = handler->error(sim_result)*scale;
                // printf("loss value: %lf!!\n", err);
                result += err;
            }
            return result;
        } else {
...
class ValueError: public ErrorHandler {
    std::vector<std::pair<SpeciesId, double>> constrains;
public:
    ~ValueError() override = default;

    ValueError() = default; 

    void add_constrain(SpeciesId id, double value) {
        this->constrains.emplace_back(id, value);
    }
    ...
    double error(std::expected<SimulationResult,double> sim_result) const override {
        ...
        double err = 0.0;
        for (const auto &pr : constrains) {
            SpeciesId id = pr.first;
            double expected = pr.second;
            std::optional<double> observed;

            if (auto it = simResult.not_constrained.find(id); it != simResult.not_constrained.end()) {
                observed = it->second;
            } else if (auto it = simResult.constrained.find(id); it != simResult.constrained.end()) {
                observed = it->second;
            }

            if (observed) {
                double d = observed.value() - expected;
                if((d*d) > 0.0001) {
                    err += d*d;
                }
            } else {
                // species missing -> heavy penalty
                printf("[FATAL ERROR] species doen't exists: %s\n", id.c_str());
                control(false);
            }
        }

        printf("[INFO] Value Error: %lf acceptable: %lf\n", err, (double)this->constrains.size()/2.0);
...
public:
    ...
    double error(std::expected<SimulationResult,double> sim_result) const override {
        ...
        for (const auto &ids : constrain) {
            ...
            auto it1 = simResult.not_constrained.find(id1);
            auto it2 = simResult.not_constrained.find(id2);

            if (it1 != simResult.not_constrained.end() && it2 != simResult.not_constrained.end()) {
                double diff = it1->second - it2->second;
                double poss_err = diff;
                if(poss_err > 0) {
                    err += poss_err;
                }
            } else {
...
public:
    ...
    double error(std::expected<SimulationResult,double> sim_result) const override {
        ...
        if(simResult.old_values.empty()) {
            printf("[FATAL ERROR] values are empty\n");
            exit(1);
        }
        // compute error iterating over species ids in old_values (unordered_map<SpeciesId,double>)
        // and looking up their new value in not_constrained or constrained (unordered_map<SpeciesId,double>)
        double err = 0.0;
        for (const auto& [species_id, old_val] : simResult.old_values) {
            double new_val;
            auto it_nc = simResult.not_constrained.find(species_id);
            if (it_nc != simResult.not_constrained.end()) {
                new_val = it_nc->second;
            } else {
                auto it_c = simResult.constrained.find(species_id);
                if (it_c != simResult.constrained.end()) {
                    new_val = it_c->second;
                } else {
                    // no new value available for this species: skip it
                    printf("[FATAL ERROR] old_values not exists");
                    exit(1);
                }
            }

            double b = (old_val - new_val);
            double fitness = std::abs(b)/(std::abs(new_val)+ std::abs(old_val) + 1e-32);
            if(fitness > 0.0001) {
                // printf("[INFO] species %s is NOT stable: %lf %lf\n", species_id.c_str(), fitness, b);
                err += fitness;
            } else {
                // printf("[INFO] species %s is stable: %lf %lf\n", species_id.c_str(), fitness, b);
            }
        }

        printf("[INFO] Transitorial Error: %lf\n", err);
