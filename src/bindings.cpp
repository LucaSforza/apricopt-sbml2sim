...
extern "C" {
    ...
    void Vector_trajectory_delete(std::vector<std::pair<std::string, std::vector<double>>> *_this) {
        delete _this;
    }
    ...
    ValueError *ValueError_create() {
        return new ValueError();
    }

    void ValueError_delete(ValueError *_this) {
        delete _this;
    }

    void ValueError_add_constrain(ValueError *_this, const char *species_id, double value) {
        control(_this && species_id);
        _this->add_constrain(std::string(species_id), value);
    }

    void SBMLDoc_contrains_on_kinetic_constants(SBMLDoc *_this, int fd) {
        _this->contrains_on_kinetic_constants(fd);
    }
}